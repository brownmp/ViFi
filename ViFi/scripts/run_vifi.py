...
if __name__ == '__main__':
    ...
    options = parse_args()
    if options.docker == True:
        print("Running with docker option")
        run_docker(options)
        exit(0)
    ########################
    # Non Docker 
    ...
    if options.bamfile is None:
        ...
        cmd = "bwa mem -t %d -M %s %s %s | samtools view -bS - > %s/%s.bam" % (options.cpus, options.reference, options.forward, options.reverse, options.output_dir, options.prefix)
        print "\n", cmd, "\n"
        ...
        # cmd = "samtools index %s/%s.bam" % (options.output_dir, options.prefix)
        # os.system(cmd)
    ...
    # os.system("samtools index %s/%s.trans.bam" % (options.output_dir, options.prefix))
    ...
    if options.disable_hmms == False:
        ...
        cmd ="python %s/scripts/run_hmms.py -t %d -b %s/%s.unknown.bam -d tmp -H %s --threshold %f" % (options.vifi_dir, options.cpus, options.output_dir, options.prefix, options.hmm_list, options.threshold)
        print "\n", cmd, "\n"
    ...
    cmd = "python %s/scripts/merge_viral_reads.py --unknown %s/%s.unknown.bam --trans %s/%s.trans.bam --reduced tmp/temp/reduced.csv --map tmp/temp/unmapped.map --output %s/%s.fixed.trans.bam" % (options.vifi_dir, options.output_dir, options.prefix, options.output_dir, options.prefix, options.output_dir, options.prefix)
    print "\n", cmd, "\n"
    ...
    cmd = "python %s/scripts/cluster_trans_new.py --data %s/%s.fixed.trans.cs.bam --output %s/%s.clusters.txt %s --min-support %d" % (options.vifi_dir, options.output_dir, options.prefix, options.output_dir, options.prefix, "" if options.chromosome_list is None else "--chrom %s" % options.chromosome_list, options.min_support)
    print "\n"
    print cmd
    print "\n"
